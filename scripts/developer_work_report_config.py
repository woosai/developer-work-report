...
Collect only each source's configured categories. Organize artifacts by their actual work date under YYYY-MM-DD with a daily index, 프롬프트/<agent>, 문서/<source-relative path>, 코드/<repository>, and 기타 as applicable. Do not create empty category folders. Assign documents by source filesystem mtime in the configured timezone and reconcile every staged date against that mtime; discovery time is not work date. Store code as date/repository patches and summaries rather than complete tracked source trees. For committed code, MUST run `{collector_script}` for the exact range and use its manifest as the source of truth. It scans `git log --all`, aggregates every unique commit before writing one repository/date patch and summary, and rejects count mismatches. It also creates one UTF-8 BOM `코드/commits.csv` per date across all repositories. Upload that CSV and reconcile its row count and SHA-256 against `daily_commit_csv` in the manifest. Its `브랜치` column is the closest current containing local/remote ref and must not be described as the historical creation branch. Never replace this with a current-branch-only log or a per-commit overwrite loop. Reconcile manifest commit hashes and counts with every daily index before upload. Extract only human prompts demonstrably associated with an enabled business source. For Codex accept only user-owned threads (`thread_source == "user"`); reject guardian/approval-review/subagent/automation events. For Claude and Antigravity reject subagent transcripts, task notifications, tool/command output, environment blocks, and compaction continuations. Never upload raw session logs, assistant/system/developer/tool content, internal reasoning, caches, build intermediates, environment files, certificates, keys, or credentials. MUST run `{sanitizer_script}` against staged prompt exports for the exact range before upload; abort on its nonzero exit. Then perform a second independent privacy inspection and withhold uncertain exports. Never modify local source files or original session logs.
