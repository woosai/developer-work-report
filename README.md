...
- 날짜별 `코드/commits.csv`에 전체 저장소 커밋 목록 통합
...
각 날짜의 `코드/commits.csv`는 Excel에서도 바로 열 수 있는 UTF-8 BOM 형식으로 생성됩니다. 시각, 저장소, 브랜치, 커밋, Conventional Commit 타입·영역, 제목, 변경 파일 수, 추가·삭제 줄 수, 머지 여부와 동일 커밋이 존재하는 다른 저장소를 한 표로 정리합니다. Git 커밋 자체에는 작성 당시 브랜치명이 저장되지 않으므로 `브랜치`는 수집 시점에 해당 커밋과 가장 가까운 로컬·원격 추적 브랜치 참조입니다.
